...
public class SlideShareAPITest extends TestCase {
    ...
    public void testGetSlideshow() throws Exception {
        ...
        assertEquals("Presentation of Groovy and Grails made at the JUG Padova on 17 May 2008", result.getDescription());
        assertEquals("grails,groovy", result.getTags());
        assertEquals("<object type=\"application/x-shockwave-flash\" data=\"http://s3.amazonaws.com/slideshare/ssplayer.swf?id=414888&doc=groovyandgrails-1211192948866493-8&branding=no\" width=\"425\" height=\"348\"><param name=\"movie\" value=\"http://s3.amazonaws.com/slideshare/ssplayer.swf?id=414888&doc=groovyandgrails-1211192948866493-8&branding=no\" /></object>", result.getEmbedCode());
        assertEquals("http://cdn.slideshare.net/groovyandgrails-1211192948866493-8-thumbnail-2", result.getThumbnail());
        assertEquals("http://www.slideshare.net/john.leach/groovy-and-grails-jug-padova", result.getPermalink());
    ...
    public void testGetSlideshowByUser() throws Exception {
        SlideShareAPI instance = new SlideShareAPIImpl(
                new MockSlideShareConnector("xml/messages/get_slideshow_by_user.xml"));
    ...
    public void testGetSlideshowByTag() throws Exception {
        SlideShareAPI instance = new SlideShareAPIImpl(
                new MockSlideShareConnector("xml/messages/get_slideshow_by_tag.xml"));
    ...
    public void testGetSlideshowByGroup() throws Exception {
        SlideShareAPI instance = new SlideShareAPIImpl(
                new MockSlideShareConnector("xml/messages/get_slideshow_by_group.xml"));
